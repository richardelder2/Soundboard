...
function handlePackChapter(chId) {
  if (!chId) {
    console.error('Error: Please specify a chapter number (e.g. node scripts/saga.js pack-chapter 1)');
    process.exit(1);
  }
...
function handleCraftSearch(searchArgs) {
  ...
  if (!query) {
    console.log(`\x1b[33mUsage: node scripts/saga.js craft search <query> [--json]\x1b[0m`);
    return;
  }
  ...
  printHeader(`SAGA Craft Search: "${query}" (${results.length} matches found)`);
...
Usage:
  ...
  node scripts/saga.js craft search <query>   Search the 80+ craft modules in _config/okf_craft/
  ...
  node scripts/saga.js pack-chapter <N>       Assemble token-disciplined drafting kit for Chapter N
  node scripts/saga.js okf-index              Rebuild index.md catalogs for OKF knowledge bundles
